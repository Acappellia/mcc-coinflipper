...
execute if score #check_prev_front ct matches 1 if data storage ct:tmp check_coin{ishead:1} unless data storage ct:tmp check_coin{coin_id:12} run return run scoreboard players set #round_check_result ct 0
...
execute if data storage ct:tmp check_coin{coin_id:12} run scoreboard players set #check_prev_front ct 0
